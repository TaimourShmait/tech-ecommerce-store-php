...
<body class="d-flex flex-column">
    ...
    <main class="d-flex flex-row w-100 p-2 gap-2 flex-grow-1 align-items-stretch">
        ...
        <nav class="admin-sidebar d-flex flex-column p-3 border rounded h-100">
            <ul class="nav d-flex flex-column sidebar-nav gap-2">
                ...
                <li class="nav-item"><a class="nav-link sidebar-link active" href="admins.php">Admins</a></li>
        ...
        <div class="main-content d-flex flex-column w-100 p-2 gap-2 flex-grow-1">
            <div class="d-flex flex-row w-100 gap-2">
                <form class="d-flex flex-grow-1 gap-2">
                    <input class="form-control flex-grow-1 p-2" type="search" placeholder="Search admins" name="search">
                    <input class="btn btn-outline-success" type="submit" value="Search">
                </form>

                <a class="btn btn-primary" href="create_admin.php"><i class="bi bi-plus-circle"></i></a>
            </div>
                    ...
                    foreach ($admins as $admin) {
                        echo "
                                <tr> 
                                    <td>" . $admin["id"] . "</td>
                                    <td>
                                        <form class='d-flex' method='POST' action='../../../api/admin/admins.php'>
                                            <input type='hidden' name='action' value='update'>
                                            <input type='hidden' name='id' value='" . $admin['id'] . "'>
                                            <input class='form-control' type='text' name='username' value='" . htmlspecialchars($admin['username'], ENT_QUOTES) . "'>
                                        <button type='submit' class='btn btn-success update-admin-btn'><i class='bi bi-pencil'></i></button>
                                        </form>
                                    </td>
                                    <td>" . $admin["created_at"] . "</td>
                                    <td>" . $admin["updated_at"] . "</td>
                                    <td>
                                        <form method='POST' action='../../../api/admin/admins.php' data-id='" . $admin["id"] . "'>
                                            <input type='hidden' name='id' value='" . $admin["id"] . "'>
                                            <input type='hidden' name='action' value='delete'>
                                            <button type='submit' class='btn btn-danger delete-btn' data-user='" . $admin['username'] . "'><i class='bi bi-trash'></i></button>
                                        </form>
                                    </td>
                                </tr>
                            ";
    ...
    <footer class="w-100 d-flex flex-row justify-content-between align-items-center p-3">
